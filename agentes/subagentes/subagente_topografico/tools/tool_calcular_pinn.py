...
def ejecutar_calcular_pinn(
    ...
    # ─── FIX-PINN-WN2 + FIX-PINN-WN2-HEAVY: carga de nieve autoritativa ────────
    # Dos casos en una sola llamada WN2:
    # (A) FALLBACK: LLM no pasó valor o pasó < 5 cm → usar p50/p95/p95_3d según umbral
    # (B) HEAVY-UPGRADE: LLM pasó valor ≥ 5 cm pero alerts_dia.heavy_snow=True
    #     → usar p95 (conservador por cola de riesgo), igual que FIX-WN2-SIZE-EAWS.
    #     Evita que p50=21.7 cm produzca FS=MARGINAL en tormentas reales donde
    #     p95=60 cm daría FS=INESTABLE y nivel EAWS correcto (e.g. 10-jun-2026).
    _fuente_nieve = None
    _MIN_NIEVE_CM_FALLBACK = 5.0
